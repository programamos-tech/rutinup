-- Fix registration function to properly handle RLS and context
-- This migration fixes the issue where users are created in auth but not in the users table

-- Drop and recreate the function with better error handling and context
DROP FUNCTION IF EXISTS create_gym_and_user(UUID, TEXT, TEXT, TEXT);
...
BEGIN
  ...
  BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM users WHERE id = p_user_id) THEN
      -- Si falla la verificación, intentar eliminar el gym creado
      BEGIN
      ...
      RETURN jsonb_build_object(
        'success', false,
        'error', 'El usuario no se pudo crear en la tabla users'
      );
    END IF;
  EXCEPTION
    WHEN OTHERS THEN
      ...
      EXCEPTION
        WHEN OTHERS THEN
          -- Si falla el delete, continuar
          NULL;
      END;
      v_error_detail := 'Error al crear usuario: ' || SQLERRM || ' (SQLSTATE: ' || SQLSTATE || ')';
      RETURN jsonb_build_object(
        'success', false,
        'error', v_error_detail
      );
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      -- Solo loguear el error pero no fallar el registro
      v_error_detail := 'Advertencia: No se pudieron crear todas las plantillas: ' || SQLERRM;
...
EXCEPTION
  ...
  WHEN OTHERS THEN
    -- En caso de error, retornar error detallado
...
GRANT EXECUTE ON FUNCTION create_gym_and_user(UUID, TEXT, TEXT, TEXT) TO anon;

-- También necesitamos asegurarnos de que la función pueda insertar en las tablas
-- Aunque SECURITY DEFINER debería ser suficiente, a veces Supabase necesita permisos explícitos
-- Las políticas RLS deberían permitir la inserción cuando se ejecuta desde esta función
